...
struct ContentView: View {
    ...
    @ObservedObject private var theme = ThemeManager.shared // Wires up the switchbox!
    ...
    private var desktopBackground: some View {
        ZStack {
            LinearGradient(
                colors: [theme.bgGradientStart, theme.bgSolid],
                        ...
                        if let outputImage = CIFilter.randomGenerator().outputImage?.cropped(to: CGRect(x: 0, y: 0, width: 300, height: 300)),
                           let cgImage = ciContext.createCGImage(outputImage, from: outputImage.extent) {
                            Image(uiImage: UIImage(cgImage: cgImage))
                                ...
                                .foregroundColor(Double.random(in: 0...1) > 0.3 ? theme.text : theme.accent)
                                .blendMode(.screen)
    ...
    private var topHeader: some View {
        HStack {
            VStack(alignment: .leading, spacing: 3) {
                ...
                // THE FIX: We wrapped the text inside an "if" statement tied to our new switch!
        ...
        .padding(.vertical, 10)
        .background(.thinMaterial.opacity(theme.isLightTheme ? 0.7 : 0.4))
                        ...
                        VStack(spacing: 6) {
                            ZStack {
                                RoundedRectangle(cornerRadius: 8)
                                    .fill(LinearGradient(colors: [theme.text.opacity(0.15), .clear], startPoint: .top, endPoint: .bottom))
                ...
                ForEach(fs.desktopShortcuts, id: \.self) { appURL in
                    Button(action: {
                        ...
                        VStack(spacing: 6) {
                            ZStack {
                                RoundedRectangle(cornerRadius: 8)
                                    .fill(LinearGradient(colors: [theme.accent.opacity(0.2), .clear], startPoint: .top, endPoint: .bottom))
                                    .frame(width: 52, height: 52)
    ...
    private var taskbar: some View {
        HStack(spacing: 12) {
            Button(action: {
                ...
                ZStack {
                    Circle()
                        .fill(RadialGradient(colors: [theme.accent, theme.bgGradientStart, theme.bgSolid], center: .center, startRadius: 0, endRadius: 22))
                    ...
                    Image(systemName: "command").font(.system(size: 16, weight: .bold)).foregroundColor(.white) // Always keep icon white so it pops
                }
            }
            
            Rectangle().fill(theme.text.opacity(0.2)).frame(width: 1, height: 26)
                ...
                HStack(spacing: 6) {
                    ForEach(pm.runningProcesses) { process in
                        ...
                        Button(action: {
                            ...
                            .background(isActive ? theme.text.opacity(0.15) : theme.bgSolid.opacity(0.2))
                            .cornerRadius(4).overlay(RoundedRectangle(cornerRadius: 4).stroke(isActive ? theme.text.opacity(0.4) : theme.text.opacity(0.1), lineWidth: 1))
                        }
                    }
                }
            }
        ...
        .overlay(Rectangle().frame(height: 1).foregroundColor(theme.text.opacity(0.25)), alignment: .top)
    }
        ...
        ZStack {
            ...
            VStack {
                ...
                HStack {
                    ...
                    .frame(width: 280)
                    .aeroGlassStyle(tint: theme.bgSolid)
                    .padding(.leading, 12)
                    .padding(.bottom, 68)
                    Spacer()
                }
    ...
    private var taskManagerOverlay: some View {
        ZStack {
            ...
            .aeroGlassStyle(tint: theme.accent)
        }
        .zIndex(100)
    }
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 25) {
                    ...
                    // THE FIX: Stacking the toggles together in a neat block
                    VStack(spacing: 12) {
                        ...
                        Toggle(isOn: $theme.showDiagnostics) {
                            Text("Diagnostics HUD: \(theme.showDiagnostics ? "VISIBLE" : "HIDDEN")")
                                .font(.system(size: 11, weight: .bold, design: .monospaced))
                                .foregroundColor(theme.text)
                        }
                        .toggleStyle(SwitchToggleStyle(tint: theme.accent))
                ...
                .background(theme.bgSolid.opacity(0.8))
            }
    ...
    struct WindowNode: View {
        ...
        @ObservedObject var theme = ThemeManager.shared // Wires up the switchbox to the windows
